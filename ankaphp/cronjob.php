...
$file1= new SplFileObject('X:\final\Participants.txt');
...
try{
    ...
    while(!$file1->eof())
    {
        ...
        file_put_contents("X:\final\Participants.txt",""); 
    }
}
...
try{
    $file2= new SplFileObject('X:\final\Products.txt');
    while(!$file2->eof())
    {
        $line2=$file2->fgets();
        ...
        $sth2->execute();   
        file_put_contents("X:\final\Products.txt","");
    }
}
...
try{
    ...
    $participantid = 0;
    $participantid = fopen('X:\final\PerformanceRequests.txt',"r") or die("Request not taken.");
    $content = fread($participantid, filesize("X:\final\PerformanceRequests.txt"));
    //empty file contents after reading to make space for more requests
    file_put_contents("X:\final\PerformanceRequests.txt","");
    ...
    $sql4 = " SELECT DISTINCT COUNT(customerName) from returning_customers where participant_id='$content'";
    ...
    while ($row4 = $result4->fetch()) {
        //echo $row1['points']."<br>";
        $returnees = $row4['COUNT(customerName)'];
    }


    $handle = fopen("X:\final\Performance.txt", "w") or die("File does not exist.");
    if(fwrite($handle,$content.";"."Points: ".$points." "."Products Left: ".$quantity." "."Rank: ".$rank." "."Returning Customers: ".$returnees) == false ){
        echo "Error Writing.";
}
}
